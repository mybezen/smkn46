...
export default function GalleriesIndex({ galleries, filters }: Props) {
    ...
    const handleSearch = (e: React.FormEvent) => {
        e.preventDefault();
        router.get('/admin/galleries', { search }, { preserveState: true, replace: true });
    };

    const handleDelete = () => {
        if (!deleteSlug) return;
        setDeleteLoading(true);
        router.delete(`/admin/galleries/${deleteSlug}`, {
            onFinish: () => {
            ...
            <motion.div className="space-y-6 p-6" variants={fadeIn} initial="hidden" animate="visible">
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-gray-500">Manage photo galleries</p>
                    </div>
                    ...
                    <CardHeader className="border-b border-gray-100 px-6 py-4">
                        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            <CardTitle className="text-base font-semibold text-gray-800 flex items-center gap-2">
                                ...
                                All Galleries
                                <Badge variant="secondary" className="ml-1 text-xs">{galleries.total}</Badge>
                            </CardTitle>
                                            ...
                                            <tr>
                                                <td colSpan={4} className="px-6 py-16 text-center">
                                                    <div className="flex flex-col items-center gap-2 text-gray-400">
                                                        <Images className="h-10 w-10 text-gray-200" />
                                                        <p className="text-sm font-medium">No galleries found</p>
                                                        <p className="text-xs">
                                                            {filters.search ? 'Try a different search term.' : 'Start by creating a gallery.'}
                                                        </p>
                                                    </div>
                                                </td>
                                            </tr>
                                            ...
                                            galleries.data.map((gallery) => (
                                                ...
                                                >
                                                    <td className="px-6 py-4">
                                                        <span className="font-medium text-gray-900 text-sm">{gallery.title}</span>
                                                    </td>
                                                    <td className="px-6 py-4">
                                                        ...
                                                        <DropdownMenu>
                                                            ...
                                                            <DropdownMenuContent align="end" className="w-36">
                                                                <DropdownMenuItem asChild>
                                                                    <Link href={`/admin/galleries/${gallery.slug}`} className="flex items-center gap-2 cursor-pointer">
                                                                        <Eye className="h-3.5 w-3.5" />
                                                                        View
                                                                    </Link>
                                                                </DropdownMenuItem>
                                                                <DropdownMenuItem asChild>
                                                                    <Link href={`/admin/galleries/${gallery.slug}/edit`} className="flex items-center gap-2 cursor-pointer">
                                                                ...
                                                                <DropdownMenuItem
                                                                    ...
                                                                    onClick={() => setDeleteSlug(gallery.slug)}
                                                                >
                                    ...
                                    {galleries.data.map((gallery) => (
                                        ...
                                        >
                                            <div className="flex flex-col gap-1 min-w-0">
                                                <span className="font-medium text-gray-900 text-sm truncate">{gallery.title}</span>
                                                <code className="rounded bg-gray-100 px-1.5 py-0.5 text-xs text-gray-500 font-mono self-start">
                                                    {gallery.slug}
                                                </code>
                                                <span className="text-xs text-gray-400">{formatDate(gallery.created_at)}</span>
                                            </div>
                                            <DropdownMenu>
                                                <DropdownMenuTrigger asChild>
                        ...
                        {galleries.last_page > 1 && (
                            <div className="flex items-center justify-between border-t border-gray-100 px-6 py-4">
                                <p className="text-sm text-gray-500">
                                    Showing {galleries.from}–{galleries.to} of {galleries.total}
                                </p>
            ...
            <Dialog open={!!deleteSlug} onOpenChange={(open) => !open && setDeleteSlug(null)}>
                <DialogContent className="sm:max-w-md rounded-2xl">
                    <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 0.2 }}>
                        <DialogHeader>
                            <DialogTitle className="text-gray-900">Delete Gallery</DialogTitle>
                            <DialogDescription className="text-gray-500">
                                This action cannot be undone. The gallery and all its images will be permanently deleted.
                            </DialogDescription>
                        </DialogHeader>
